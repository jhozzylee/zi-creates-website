...
const PaymentModal = ({ isOpen, onClose, selectedPlan }: PaymentModalProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      fetch("https://ipapi.co/json/")
        .then((res) => res.json())
        .then((data) => setFormData((prev) => ({ ...prev, phone: data?.country_calling_code || "+1" })))
  ...
  const config = {
    ...
    customer: { email: formData.email, name: formData.company },
    customizations: {
    ...
    callback: (response: any) => {
      ...
      // Sync to Make and SheetDB
      Promise.all([
        fetch("https://hook.us2.make.com/24c46m8vplnnst93l3xlba6sakukrkj6", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }),
        fetch("https://sheetdb.io/api/v1/sfstsm92m5k39", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ data: payload }) })
      ]);
          ...
          {submitted ? (
            ...
            <>
              <div className="mb-12">
                ...
                <h2 className="text-3xl md:text-5xl font-bold tracking-tighter">Let’s get you <span className="text-neutral/30 italic font-light">started</span></h2>
              </div>
              ...
              <form className="space-y-10" onSubmit={handlePay}>
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-8 pt-6 border-t border-neutral/5">
                  ...
                  <div className="w-full md:w-auto min-w-[240px]">
                    <CTAButton 
                      text={isPaying ? "Connecting..." : `Secure Payment`} 
                      type="submit" 
                      disabled={isPaying} 
                    />
                  </div>
  ...
  <div className="flex flex-col gap-2 group">
    ...
    <div className="relative flex items-center">
      ...
      >
        ...
        {options.map((opt: string) => <option key={opt} value={opt} className="bg-[#0a0a0a]">{opt}</option>)}
      </select>
    </div>
  </div>
);
